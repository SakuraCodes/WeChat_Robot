...
# 去除重复行
selected_df1 = selected_df1.drop_duplicates(subset=['商家名称', '满返'],
                                            keep='first')
selected_df2 = selected_df2.drop_duplicates(subset=['店铺名称', '经营品类'],
                                            keep='first')

# 合并数据(根据相同列进行匹配)
merged_df = pd.merge(selected_df1, selected_df2, left_on='商家名称',
                     right_on='店铺名称', how='left')

# 去除匹配列(axis=0表示删除行，axis=1表示删除列)
merged_df = merged_df.drop(['商家名称', '店铺名称'], axis=1)
